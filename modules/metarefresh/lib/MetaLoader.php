...
class sspmod_metarefresh_MetaLoader {
	...
	public function loadSource($source) {
		...
		foreach($entities as $entity) {
			...
			if(isset($source['entity-attributes']) && !empty($source['entity-attributes'])) {
				// Allow any entity category, it is used when we would like to exclude entities having some of the enityt category
				if (in_array('*', $source['entity-attributes'])) {
                                	$present = true;
                                }
				else {
					$present = false;
				}
				// Get all entity categories from the entity
				if(!empty($entity->getEntityAttributes()) && is_array($entity->getEntityAttributes())) {
					// Get entity category name and its values
					foreach ($entity->getEntityAttributes() as $entityAttributeName => $entityAttributeValue) {
						// Iterate through the configuration and match only those which are in config and attached to the entity
						if (in_array($entityAttributeName, array_keys($source['entity-attributes']))) { 
							if (is_array($entityAttributeValue)) {
								foreach($entityAttributeValue as $entityAttributeSubValue) {
									if (isset($source['entity-attributes'][$entityAttributeName]) && is_array($source['entity-attributes'][$entityAttributeName]) && isset($entityAttributeSubValue)) {
										if (in_array($entityAttributeSubValue, $source['entity-attributes'][$entityAttributeName])) {
											$present = true;
										}
										if (in_array('!' . $entityAttributeSubValue, $source['entity-attributes'][$entityAttributeName])) {
											$present = false;
					...
					if (!$present) {
						SimpleSAML_Logger::info('Skipping "' .  $entity->getEntityID() . '" - does not have proper entity attribute.' . "\n");
						continue;
					}
				} elseif (!in_array('*', $source['entity-attributes'])) { // Skip only if * hasn't been used.
					SimpleSAML_Logger::info('Skipping "' .  $entity->getEntityID() . '" - does not have any entity attribute.' . "\n");
					continue;
				}
			}
