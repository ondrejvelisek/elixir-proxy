...
class sspmod_smartattributes_Auth_Process_SmartID extends SimpleSAML_Auth_ProcessingFilter {
	...
	/**
	 * Name of eduPersonTargetedID attribute which needs
	 * to be treated specialy if it contains NameID value.
	 */
	private $eduPersonTargetedIDName = 'eduPersonTargetedID';
	...
	private function addID($attributes, $request) {
		foreach ($this->_candidates as $idCandidate) {
			if (isset($attributes[$idCandidate][0])) {
				$attributeValue = $attributes[$idCandidate][0];
				if ($idCandidate == $this->eduPersonTargetedIDName) {
					$attributeValue = $this->convertEduPersonTargetedIDToString($attributeValue);
				}
				$userId = '';
				if(($this->_add_authority) && (isset($request['saml:AuthenticatingAuthority'][0]))) {
					$userId = ($this->_add_candidate ? $idCandidate.':' : '').$attributeValue . '!' . $request['saml:AuthenticatingAuthority'][0];
				} else {
					$userId = ($this->_add_candidate ? $idCandidate.':' : '').$attributeValue;
				}

				SimpleSAML_Logger::debug('Found candidate \'' . $idCandidate . '\' with value \'' . $userId . '\'');
				return $userId;
			}
		}
		/*
	...
	/**
	* Convert eduPersonTargetedID NameID value into the text representation.
	*/
	private function convertEduPersonTargetedIDToString($attributeValue) {
		if (is_object($attributeValue) && get_class($attributeValue) == "DOMNodeList") {
			$node = $attributeValue->item(0);

			$nameQualifier = $node->attributes->getNamedItem('NameQualifier')->nodeValue;

			$spNameQualifier =  $node->attributes->getNamedItem('SPNameQualifier')->nodeValue;

			$value = $node->nodeValue;

			return $nameQualifier . '!' . $spNameQualifier . '!' . $value;	
		} else {
	...
	{
		SimpleSAML_Logger::info('SmartID: '.$message);
	}
